...
mean_sd = df %>% filter(as.Date(HourDK)>=Sys.Date()-days(30)) %>% dplyr::summarize(mean_=mean(SpotPriceDKK),sd_=sd(SpotPriceDKK))
...
# Best windows
msg_ = res %>% 
  dplyr::mutate(label_=glue("{window}H Window: {hour(HourDK)}:00 - {scales::number(rollingsum,accuracy=0.001)} DKK ({p_label})")) %>% 
  pull(label_) %>% 
  paste(.,collapse="\n")
