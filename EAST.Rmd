...
# Mean and SD
mean_sd = df %>% 
  filter(HourDK >= Sys.time()-months(1)) %>% 
  dplyr::mutate(SpotPriceDKK_z=scale(SpotPriceDKK)) %>% 
  dplyr::summarise(mean_=mean(SpotPriceDKK),
                   sd_=sd(SpotPriceDKK))
...
res <- psel(df_windows, p, top = nrow(df_windows)) %>% filter(.level==min(.level)) %>% 
  dplyr::mutate(p_score=pnorm(q = rollingsum,mean = mean_sd$mean_,sd = mean_sd$sd_))
res %>% 
...
# Best windows
msg_ = res %>% 
  dplyr::mutate(label_=glue("{window}H Window: {hour(HourDK)}:00 - {scales::number(rollingsum,accuracy=0.001)} DKK ({scales::percent(p_score,0.1)})")) %>% 
  pull(label_) %>% 
  paste(.,collapse="\n")
